...
- 進行中レーンのWIP制限（同時に5件まで。超過時はUIで警告しブロック）
...
`PATCH /api/tasks` は所有権チェック（`updateMany` で `id` と `userId` の両方に一致する行のみ更新）と更新後の再取得を `prisma.$transaction` で1つのトランザクションにまとめています。2クエリに分かれたままだと、間に別リクエストが割り込むTOCTOUギャップが理論上残るためです。同じトランザクション内で、`IN_PROGRESS`への遷移時はWIP制限のカウントチェックも行っています（後述）。
...
### WIP制限・保留レーン・優先度・停滞検知

タスク管理を「作業を絞り込む」「他者待ちを可視化する」「優先度と停滞を意識する」という観点で強化する4つの追加機能です。

- **WIP制限**（`lib/constants.ts` の `IN_PROGRESS_WIP_LIMIT`）: 進行中レーンを同時に5件までに制限。クライアント側（`useKanban.ts`）で早期にブロック・警告表示する一方、複数タブや直接APIを叩く経路も塞ぐため `PATCH /api/tasks` のトランザクション内でも同じ上限をカウントチェックし、超過時は409を返します。定数を共有ファイルに集約し、クライアントとサーバーの値がずれないようにしています
- **保留（PENDING）レーン**: `TaskStatus` enumに`PENDING`を追加するマイグレーションを実施。レーン順序を配列（`TASK_STATUS_ORDER`）として一元管理し、クイック移動ボタンの前後判定・WIP制限のロジックがレーン数の変化に対して汎用的に動くようにしています
...
タスクのステータスは `TODO`（未着手）, `IN_PROGRESS`（進行中）, `PENDING`（保留）, `DONE`（完了）の4種類です。`IN_PROGRESS`への更新はサーバー側でもWIP制限（進行中5件まで）を強制しており、超過時は409を返します。
