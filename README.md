...
### UIデザイン（Liquid Glass風の軽量CSS実装）

Apple WWDC25の「Liquid Glass」はSVGディスプレイスメントフィルタを使うリアルタイム屈折素材で、Chromium以外のブラウザでは`backdrop-filter`がSVGフィルタ入力を受け付けず動作しない上、描画コストも高いです。そのため本物の再現ではなく、`backdrop-filter: blur()`＋半透明＋ハイライト境界線だけの軽量なCSS近似を採用しています（トークン設計や理由の詳細は [LIQUID_GLASS_DESIGN.md](LIQUID_GLASS_DESIGN.md) 参照）。背景の初期案は青・紫・ピンクの3色グラデーションでしたが、AIプロダクトのマーケティングページで頻出する構成だと判断し、ブランドカラー1色の淡いグラデーションに変更しています。
...
ここから下は、ローカルで実際にセットアップ・動作確認したい場合の手順です。[デプロイ済みのURL](https://nextjs-app-ta5aoszrnq-an.a.run.app)でゲストログインしてすぐ試すこともできます。
...
npm run test:coverage    # ユニットテストをカバレッジ計測付きで実行
...
ログイン済みユーザーのタスクタイトルを Gemini API に送り、カテゴリを推定します。同一タイトルの結果はGeminiの`responseSchema`検証に加えてサーバー側でもZodで再検証してからプロセス内メモリにキャッシュされます（不正な値をキャッシュしない）。
...
# Auth.jsのredirect_uri構築に使う正式な公開URL。Cloud Runのデフォルトドメインは
# 初回デプロイ後に確定するため、別プロジェクトで使う場合は実際のURLに差し替える
