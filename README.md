...
- **インフラ**: Terraformの変更は `terraform validate` / `terraform plan` で構文・整合性を事前確認してから `apply` している。`plan` で意図しない差分（例: Cloud Runのイメージが本番のものからTerraformコード上のプレースホルダーに巻き戻る等）が出た場合は、影響範囲を `-target` で絞るか、Terraform管理外の `gcloud` コマンドで代替するかたちで安全側に倒している
...
Auth.jsのResendプロバイダーでメールリンク（マジックリンク）ログインを一度実装しましたが、最終的に丸ごと削除しました。理由は、実際に運用するにはResend側でドメイン認証が必要で、それをしない限り自分の検証済みメールアドレスにしか送信できず、ログイン画面にボタンとフォームだけが存在して押しても機能しない状態になっていたためです。UIに露出していて訪問者が実際に踏める導線だったので、「動かないものを見せる」より「削除して、Google OAuth・ゲストログインの2経路に絞る」方を選びました。認証プロバイダーを複数構成できる設計自体は元のコード・コミット履歴で示せるため、削除しても実装力の証明としては失っていないと判断しています。
...
- ゲストが正式アカウント（Google）に切り替えられるよう、`/login` はゲストセッション中でもフォームを表示するようにしています（通常ログイン済みの場合のみスキップ）。`BoardHeader` にもゲスト時だけ「ログイン」リンクを出しています。なお、ゲストから正式アカウントへの切り替えはタスクデータを引き継ぎません（別ユーザーとして作成されるため）——現状は許容している制限です
- 旧 `/app` への直リンク・ブックマークは `proxy.ts` で `/` へリダイレクトしています

### Gemini APIキーをCloud Run/DBとは別プロジェクトに分離した理由

Google CloudのGemini APIには、他の多くのGCPサービス（BigQuery、Cloud Storageなど）と違う特有の挙動があります。**プロジェクトに一度でも請求先アカウントをリンクすると、そのプロジェクトのGemini API無料枠は即座に失われ、以降は初回トークンから課金対象になります。**

Cloud Run・Cloud SQLを動かすには請求先アカウントのリンクが必須なので、当初は単一プロジェクト（`my-app-portfolio`）にすべてをまとめていましたが、これによりGemini APIも無料枠を失い、「前払いクレジットの枯渇」で分類・トリアージ機能が429エラーで止まる事態になりました。

対応として、Gemini APIキーの発行元だけを別プロジェクト（`my-app-gemini`、請求先アカウント未リンク）に分離しました。

- Cloud Run・Cloud SQL・ドメインなどインフラ一式は引き続き `my-app-portfolio` が担い、Gemini APIキーだけ別プロジェクトから発行して環境変数（`GEMINI_API_KEY`）として渡しています。プロジェクトをまたいでも、APIキーは単なる文字列なのでコード変更は不要です
- 無料枠のレート制限（Lite系モデルで15 RPM / 500 RPD）は、有料ティア（4,000 RPM）と比べるとかなり狭いですが、分類・優先度判定・重複検出という軽量なタスク特性とポートフォリオ規模のトラフィックを踏まえると、無料枠で十分と判断しました
- レート制限はモデル単位でカウントされるため、`/api/classify`（`gemini-3.5-flash-lite`、タスク追加のたびに呼ばれる高頻度用途）と `/api/triage`（`gemini-3.1-flash-lite`、オンデマンドの低頻度用途）であえて異なるモデルを使っています。同一モデルに揃えると1つの500 RPD枠を取り合う形になりますが、モデルを分けることで枠も分離され、合計の実質使用可能量が2倍（RPD 1000 / RPM 30）になり、かつ高頻度な分類呼び出しが低頻度なトリアージの枠を圧迫しない構成にしています
- デメリットとして、管理対象のプロジェクト・APIキーが1組増え、Terraformの管理外の手動設定が1つ増えます。「アーキテクチャ上の必然」ではなく「無料枠を維持するためのコスト回避」であることは、面接等で聞かれたら正直に説明する前提です
- なお、Google Cloudは組織内に既存の請求先アカウントがあると、新規プロジェクトにも自動でリンクしてしまうことがあります。実際に本プロジェクトでもこれが発生し、請求先アカウントの「アカウント管理」画面から明示的にリンク解除（課金を無効にする）することで無料枠に戻しました
